...
def _page(slide, n):
    ...
    _text(tf, f"{n}/10", 11, GREY, align=PP_ALIGN.RIGHT)
...
я сталкиваюсь в работе аналитика девелопера. Пять минут — поехали.""")
    ...
    _page(s, 2)
    _notes(s, """[0:15–0:50] Проблема простая и реальная. Аналитики вручную относят
...
можно только по уличной панораме. Отсюда идея проекта.""")
    ...
    _page(s, 3)
    _notes(s, """[0:50–1:25] Решение: берём координаты участка из GeoPackage, находим
...
проверяет и правит результат. То есть это не просто модель, а рабочий продукт.""")
    ...
    _page(s, 4)
    _notes(s, """[1:25–2:05] Про данные — здесь была основная работа. Первое: почти
...
меток, а не архитектура, дало основной прирост.""")
    ...
    _page(s, 5)
    _notes(s, """[2:05–2:45] Как это работает под капотом. По контуру участка находим
...
финальный класс выбираем голосованием с весом по уверенности.""")
    ...
    _page(s, 6)
    _notes(s, """[2:45–3:30] Результаты. Продакшен-модель на честной валидации со
...
VLM-метки поднял точность до 76%. То есть главный рычаг — качество данных.""")
    ...
    _page(s, 7)
    _notes(s, """[3:30–4:00] Важно быть честным. Если сравнивать в лоб на всём GPKG,
...
сдвиг домена и маленькие выборки. ML — это дополнение к тексту на сложных случаях.""")
    ...
    _page(s, 8)
    _notes(s, """[4:00–4:30] Теперь продукт. Аналитик загружает GeoPackage, видит
