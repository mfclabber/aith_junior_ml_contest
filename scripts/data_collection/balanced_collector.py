#!/usr/bin/env python3
"""
Сбалансированный сбор датасета: гарантирует минимум N панорам, равномерно распределенных по классам.

Использование:
    python3 balanced_collector.py --target-panoramas 5000 --output-dir data/balanced_dataset
"""
    ...
    for item_dir in class_dir.iterdir():
        ...
        if metadata_path.exists():
            try:
                ...
                # Если есть panoramas_by_season, считаем их (до 4 панорам на объект)
        ...
        if count == 0 or not metadata_path.exists():
...
def main():
    parser = argparse.ArgumentParser(
        description="Сбалансированный сбор датасета с гарантией минимума панорам",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Примеры:
  # Собрать минимум 5000 панорам, равномерно по классам
  python3 balanced_collector.py --target-panoramas 5000 --output-dir data/balanced
  
  # Собрать 10000 панорам с начальным лимитом 2000 объектов на класс
  python3 balanced_collector.py --target-panoramas 10000 --initial-max-results 2000
        """
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/balanced_dataset",
        help="Директория для сохранения (по умолчанию: data/balanced_dataset)"
    )
    ...
    print(f"\n{'='*70}")
    print(f"СБАЛАНСИРОВАННЫЙ СБОР ДАТАСЕТА")
    print(f"{'='*70}")
    ...
    print("ПЕРВЫЙ ПРОХОД: сбор всех классов")
    print("-" * 70)
    ...
    # Итеративная досборка для классов с недостатком
    iteration = 0
    while iteration < args.max_iterations:
        ...
        for class_name, needed in needs_more:
            # Увеличиваем лимит пропорционально недостатку
            # Примерно 2-3 OSM объекта дают 1 панораму (с учетом сезонов может быть больше)
